...
public class ThrowHelperTests
{
    ...
    public void ThrowIfDifferentLengthSpan(int[] baseline, int[] correctLength, int[] wrongLength)
    {
    ...
    [TestCase(new[] { 1, 2, 3, 4, 5 }, new[] { 5, 4, 3, 2, 1 }, new[] { 1, 3, 5 })]
    public void ThrowIfDifferentLengthVector(int[] baseline, int[] correctLength, int[] wrongLength)
    {
    ...
    public bool ThrowIfDifferentSize(Matrix<int> matrix1, Matrix<int> matrix2) 
    ...
    private static IEnumerable<TestCaseData> IndexRangeMatrices()
    {
        ...
        yield return new TestCaseData(matrix, 0, 0, false, false).SetName("Mat3x5 at (0,0)");
        yield return new TestCaseData(matrix, 2, 4, false, false).SetName("Mat3x5 at (2,4)");
        yield return new TestCaseData(matrix, 2, 5, false, true).SetName("Mat3x5 at (2,5)");
        yield return new TestCaseData(matrix, 3, 4, true, false).SetName("Mat3x5 at (3,4)");
        yield return new TestCaseData(matrix,  2, -1, false, true).SetName("Mat3x5 at (2,-1)");
        yield return new TestCaseData(matrix, -1, 2, true, false).SetName("Mat3x5 at (-1,2)");

        matrix = [[]];
        yield return new TestCaseData(matrix, 0, 0, true, true).SetName("Mat0x0 at (0,0)");
    }
    
    [TestCaseSource(nameof(IndexRangeMatrices))]
    public void IndexRangeMatrix(Matrix<int> matrix, int i, int j, bool iInvalid, bool jInvalid)
    {
    ...
    private static IEnumerable<TestCaseData> IndexRangeVectorSet()
    {
        ...
        yield return new TestCaseData(columnVector, 0).Returns(false).SetName("ColumnVector3 at 0");
        yield return new TestCaseData(columnVector, -1).Returns(true).SetName("ColumnVector3 at -1");
        yield return new TestCaseData(columnVector, 3).Returns(true).SetName("ColumnVector3 at 3");
        
        RowVector<int> rowVector = [1, 2, 3];
        yield return new TestCaseData(rowVector, 0).Returns(false).SetName("RowVector3 at 0");
        yield return new TestCaseData(rowVector, -1).Returns(true).SetName("RowVector3 at -1");
        yield return new TestCaseData(rowVector, 3).Returns(true).SetName("RowVector3 at 3");
    }

    [TestCaseSource(nameof(IndexRangeVectorSet))]
    public bool IndexRangeVector(AbstractVector<int> vector, int i)
    ...
    {
        yield return new TestCaseData((Matrix<int>) [[]]).Returns(true);
        yield return new TestCaseData((Matrix<int>) [[1]]).Returns(false);
        
        yield return new TestCaseData((ColumnVector<int>) []).Returns(true);
        yield return new TestCaseData((ColumnVector<int>) [1]).Returns(false);
        
        yield return new TestCaseData((RowVector<int>) []).Returns(true);
        yield return new TestCaseData((RowVector<int>) [1]).Returns(false);
    }

    [TestCaseSource(nameof(EmptyMatrices))]
    public bool ThrowIfEmpty(IRectanglarMatrix<int> matrix)
